  ...
  return (
    <div className="flex justify-center flex-col mt-28 items-left ml-[10rem] ">
      <div className="w-full overflow-x-auto sm:-mx-6 lg:-mx-8 border-4 rounded-md px-4 py-4 mr-[10rem]">
        ...
        {mod && <ModifyIngredient mod={mod} setMod={setMod} setInv={setInv} ingredient={ingredient}/>}
        <div className="flex items-center ml-[1rem] mb-[1rem]">
          <button id="재료추가" className={buttonDesign()} onClick={isaddIngredient}>
            재료추가
          </button>
        </div>
        <table className="min-w-full text-center text-sm font-light border-t-2 border-black">
          ...
          <tbody>
            {Inv.map((item: IngredientDto, idx: number): any => {
              ...
              // 날짜 차이 계산 (밀리초 단위)
              const timeDiff: number = expiredDate.getTime() - buyDate.getTime();
              ...
              return (
                <tr key={idx.toString()} className="border-b dark:border-neutral-500">
                  ...
                  <td className="whitespace-nowrap  px-6 py-4 text-right">{item.primePrice}</td>
                  <td className="whitespace-nowrap  px-6 py-4 text-right">
                    {/* buy quantity */}
                  ...
                  <td className="whitespace-nowrap  px-6 py-4 text-right">{item.url}</td>
                </tr>
              );
            })}
          </tbody>
